...
if [[ $CHOICE == "y" ]]; then
    ...
    apt-get install -y git subversion curl irb rsync emacs emacs-goodies-el php-elisp nmap fail2ban denyhosts httperf siege tree unzip gcc build-essential linux-headers-$(uname -r) python task ruby libruby ruby-dev rubygems libmysql-ruby libactiverecord-ruby htop sloccount
