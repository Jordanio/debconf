...
if [[ $CHOICE == "y" ]]; then
    ...
    apt-get install -y git subversion curl irb rsync emacs emacs-goodies-el php-elisp tree unzip gcc build-essential linux-headers-$(uname -r) python ruby libruby ruby-dev rubygems libmysql-ruby libactiverecord-ruby libxslt-dev libxml2-dev htop sloccount aria2 rar p7zip
